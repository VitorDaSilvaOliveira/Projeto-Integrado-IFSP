...
\begin{quadro}[htb]
\caption{\label{quadro_rf}Requisitos Funcionais (RF01 a RF08)}
\begin{tabular}{|p{1.0cm}|p{2.6cm}|p{4.0cm}|p{7.0cm}|}
    \hline
...
\begin{quadro}[htb]
\caption{\label{quadro_rnf}Requisitos Não Funcionais (RNF01 a RNF06)}
\begin{tabular}{|p{1.2cm}|p{2.8cm}|p{3.0cm}|p{8.6cm}|}
    \hline
    \textbf{Código} & \textbf{Atores} & \textbf{Nome} & \textbf{Descrição} \\ \hline

    RNF01 & Desenvolvedores & Usabilidade & O sistema deve possuir uma interface simples, intuitiva e de fácil navegação, facilitando o uso tanto em computadores quanto em dispositivos móveis. \\ \hline

    RNF02 & Desenvolvedores & Desempenho & O sistema deve apresentar tempo de resposta satisfatório, com carregamento de páginas e listagens em no máximo 2 segundos em conexões regulares. \\ \hline

    RNF03 & Sistema & Segurança & O sistema deve implementar autenticação de usuários, controle de sessões e validação de entrada de dados para prevenir falhas como injeção de código e acessos não autorizados. \\ \hline

    RNF04 & Sistema & Backup & O sistema deve realizar backups periódicos automáticos para garantir a integridade e recuperação dos dados em caso de falhas ou perdas. \\ \hline

    RNF05 & Desenvolvedores & Escalabilidade & O sistema deve ser projetado de forma modular, permitindo futuras expansões e adaptações sem reestruturações complexas. \\ \hline

    RNF06 & Desenvolvedores & Portabilidade & O sistema deve ser compatível com diferentes navegadores modernos e adaptável a diferentes tamanhos de tela (responsivo). \\ \hline
