...
\begin{tabular}{|l|l|l|}
\hline
\textbf{SDM} & \textbf{SMR} & \textbf{Team Member} \\ \hline
Vitor        & Beatriz      & Malheus             \\ \hline
             & Rafael       & Klaith              \\ \hline
             &              & Gean                \\ \hline
             &              & Marcelo             \\ \hline
             &              & Rafael              \\ \hline
...
\begin{itemize}
    \item \textbf{SDM (Software Development Manager)}: 
    Responsável por gerenciar as equipes de desenvolvimento, garantindo que o desenvolvimento do software esteja alinhado com os objetivos do projeto. Ele toma decisões sobre o planejamento, coordenação e execução das atividades técnicas e gerenciais.
    
   \item \textbf{SMR (Software Methodology Responsible)}: 
    Responsável por zelar pela aplicação correta da metodologia Kanban. Atua monitorando o fluxo contínuo de tarefas, identificando gargalos, promovendo a melhoria contínua e incentivando a transparência e colaboração na equipe.

    \item \textbf{Team Member}: 
    São os desenvolvedores que realizam o trabalho técnico e colaboram na entrega de funcionalidades e tarefas específicas dentro do projeto. Eles atuam na implementação, testes, revisão de código e outras atividades técnicas.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Manutenção Corretiva}: correção de falhas no uso do sistema, foram encontrados algumas ações do sistema em inconformidade com as regras de negócio estabelecidas, algumas inconsistências nos formulários. Também foram identificadas algumas chamadas incorretas no código C\#  
    \item \textbf{Manutenção Evolutiva}: conforme se deu o desenvolvimento, percebemos que algumas outras necessidades surgiram, como mudança em alguns campos e em alguns fluxos, como por exemplo o campo de observação da movimentação.
...
\qquad - Hospedagem e banco de dados & - & 0,00 \\
\midrule
\textbf{Total mensal} & & \textbf{R\$ 0,00} \\
\bottomrule
\end{tabular}
\fonte{Elaborado pelos autores.}
\end{table}
...
Descrição Geral: A loja faz controle de estoque manualmente, por anotações em papel feitas pelo dono.

Etapas do Processo:

1. Recebimento de produtos (entrada): O dono recebe os produtos de fornecedores. Ele anota em um caderno: nome do produto, quantidade recebida, data e fornecedor. 

2. Venda de produtos (saída): Ao vender um produto, o dono anota à mão no mesmo caderno: nome do produto, quantidade vendida, data e valor da venda.

3.Controle de estoque: O dono consulta o caderno para verificar a quantidade de produtos disponíveis. Não há atualização automática; o controle depende da leitura e interpretação das anotações.

4. Relatório mensal: O dono folheia o caderno, soma manualmente as entradas e saídas e tenta fazer um resumo no final do mês. Processo demorado e sujeito a erros.

Problemas Identificados:

• Alto risco de erro humano.
• Dificuldade em rastrear movimentações específicas.
• Falta de relatórios precisos.
• Perda ou danos ao caderno comprometem todo o controle.
• Nenhuma visibilidade em tempo real do estoque.

Processo TO BE (Situação Proposta com Sistema)

Descrição Geral: Será implementado um sistema informatizado para registrar entradas, saídas e gerar relatórios mensais automaticamente.

Etapas do Processo com o Sistema:

1. Recebimento de produtos (entrada): O dono ou funcionário acessa o sistema. Registra os produtos recebidos: nome, quantidade, fornecedor, data, nota fiscal. O estoque é atualizado automaticamente.

2 .Venda de produtos (saída): No momento da venda, o produto é registrado como “vendido” no sistema. O sistema subtrai automaticamente a quantidade vendida do estoque.

3. Controle de estoque: O sistema exibe em tempo real o estoque disponível de cada item. Alertas de baixo estoque podem ser configurados.

4.Relatório mensal de movimentações: Ao fim do mês (ou sob demanda), o sistema gera automaticamente um relatório com:

o Total de entradas por produto.
o Total de saídas por produto.
o Saldo atual.
o Produtos mais vendidos.
o Histórico por período.

Benefícios Esperados:

• Redução de erros manuais.
• Facilidade no rastreamento de movimentações.
• Economia de tempo.
• Maior controle e visibilidade sobre o estoque.
• Dados organizados e acessíveis a qualquer momento.
