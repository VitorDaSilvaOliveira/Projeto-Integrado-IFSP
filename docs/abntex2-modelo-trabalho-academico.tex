...
\begin{itemize}
    ...
    \item Implementar um módulo de cadastro e gerenciamento de clientes, integrando o registro de vendas ao histórico individual do consumidor, para auxiliar na análise de padrões de compra.
\end{itemize}
...
A equipe optou pela utilização do método \textbf{Scrum}, junto da ferramenta \textbf{Kanban} para o gerenciamento das atividades do projeto, e o uso de \textbf{Sprints} para controlar o cronograma de atividades.
...
\begin{itemize}
    ...
    \item \textbf{Em Revisão:} Etapa de verificação e revisão da tarefa antes da finalização.
...
Vitor        & Beatriz      & Matheus             \\ \hline
             ...
             &              & Rafael              \\ \hline
\end{tabular}
\caption{Atribuição de Papéis da Equipe}
\end{table}
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.3\textwidth]{Figuras/site.png}
    \caption{QR Code para a aplicação - recomendado acessar no PC}
\end{figure}

\begin{center}
    \href{https://estoqueifsp.azurewebsites.net}{https://estoqueifsp.azurewebsites.net}
\end{center}
...
\subsection{Cronograma de Atividades – Segundo Semestre}

No segundo período de desenvolvimento, o foco maior está no desenvolvimento da da aplicação em si, aprimorando o código, criando novas funções, melhorando padrões de segurança e transformando o protótipo no resultado final esperado. Esse semestre foi guiado por \textit{Sprints} de duas semanas, e em cada uma delas houve um foco maior em tal área do desenvolvimento. A seguir, uma breve sintese do foco de cada \textit{Sprint}:

\begin{itemize}

\item Sprint 1 (04/09 - 17/09): Elicitação de requisitos e Regras de Negócio.

\item Sprint 2 (18/09 - 01/10): Implementação de novos fluxos de processo.

\item Sprint 3 (02/10 - 15/10): Validação de números e Regras de Negócio.

\item Sprint 4 (16/10 - 29/10): Bateria de testes sistemática e implementação de processos finais.

\item Sprint 5 (30/10 - 12/11): Implementação de Relatórios e "Multi-lojas".

\item Sprint 6 (13/11 - 26/11): Ajustes e Correções finais.


\end{itemize}
\chapter{Viabilidade Financeira}

O presente capítulo visa demonstrar a viabilidade econômica do sistema de gerenciamento de estoque, indo além do mero levantamento de custos e apresentando uma análise completa do retorno sobre o investimento. Para tal, serão simulados e detalhados os custos de desenvolvimento e operacionais, quantificados os benefícios gerados pela otimização do processo e calculados os principais indicadores financeiros, como o \textit{Payback}.
...
\begin{table}[hbtp]
    ...
    \begin{tabular}{|l|c|c|c|c|c|}
        ...
        \hline
        \textbf{Total de Pessoas} & \textbf{7} & - & - & \textbf{61.500,00} \\
        \hline
    \end{tabular}
...
\section{Receita/Economia Gerada}
...
\begin{table}[hbtp]
    \centering
    \caption{Projeção Anual de Benefícios Quantificáveis}
    \label{tab:beneficios}
    % Usa tabularx para a largura total da página
    \begin{tabularx}{\textwidth}{>{\centering}p{3cm} X >{\raggedleft\arraybackslash}p{2.5cm}}
        \toprule
        \textbf{Benefício} & \textbf{Descrição/Cálculo (Simulado)} & \textbf{Valor Anual (R\$)} \\
        \midrule
        ...
        \midrule
        \textbf{Total Anual de Benefícios Brutos} & \multicolumn{1}{r}{\textbf{Subtotal}} & \textbf{42.200,00} \\
        \bottomrule
    \end{tabularx}
    \fonte{Elaborado pelos autores.}
\end{table}
