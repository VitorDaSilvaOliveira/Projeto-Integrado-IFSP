...
\begin{itemize}
    ...
    \begin{itemize}
        \item \textbf{Convenções de Nomenclatura:} Aplicação de \textit{PascalCase} para nomes de classes, métodos, propriedades (e.g., \texttt{DashboardPedidosController}) e \textit{camelCase} para variáveis locais (e.g., \texttt{faturamentoAcumulado}).
        ...
        \item \textbf{Tipagem e Performance:} Uso de construtores primários em Services (e.g., \texttt{DashboardPedidosService(EstoqueDbContext context)}) e utilização de \textit{Records} ou \textit{Value Tuples} para retorno eficiente de múltiplos valores (e.g., \texttt{Task<(List<string> labels, List<int> vendas, List<int> trocas)>}).
    \end{itemize}
    ...
    \begin{itemize}
        \item \textbf{Controle e Roteamento:} Utilização de \textit{Areas} (\texttt{/Areas/Estoque/}) para organizar módulos grandes e \textit{Controllers} (Ex: \texttt{DashboardPedidosController.cs}) para roteamento e orquestração.
        ...
        \item \textbf{Consulta a Dados (EF Core):} A manipulação de coleções (\texttt{Include}, \texttt{SelectMany}) e as conversões de tipos (Ex: \texttt{MapStatus}) foram forçadas para a execução no cliente (\texttt{.ToListAsync()}) para evitar o erro de projeção \textit{"InvalidOperationException: The client projection contains a reference..."}.
    \end{itemize}
\end{itemize}

\section{Cobertura de Código e Qualidade} % Exemplo: 4.4 Cobertura de Código e Qualidade
...
\subsection{Metas e Escopo}
A estratégia de cobertura foi focada em garantir a validação rigorosa da \textbf{Lógica de Negócio} e das interações com o banco de dados.

\begin{itemize}
    \item \textbf{Meta de Cobertura:} Foi estabelecida uma meta mínima de \textbf{80\% de cobertura de linhas} (\textit{Line Coverage}) para todos os módulos que contêm regras de negócio e complexidade algorítmica.
    \item \textbf{Foco da Cobertura:} O esforço principal de teste é concentrado nas camadas de \textbf{Lógica de Negócio (\textit{Services})} e nas \textbf{Regras de Domínio (\textit{Entities})}, onde se encontram os cálculos críticos de faturamento e os mapeamentos de status.
\end{itemize}

\subsection{Implementação e Validação}
A Cobertura é assegurada por meio da aplicação de padrões de testes no \textit{Back-end} do C\#.

\begin{itemize}
    \item \textbf{Framework de Teste:} Utilização de \textit{frameworks} de teste primário, integrado ao \textit{pipeline} de construção do projeto.
    \item \textbf{Validação da Lógica de Dados:} Os testes unitários são críticos para validar as classes que contêm regras de agregação de dados do Entity Framework Core (EF Core), como a \texttt{DashboardPedidosService}.
    \item \textbf{Garantia de Cálculo:} Os testes garantem que, por exemplo, a agregação de faturamento está correta, validando que:
        \begin{itemize}
            \item A consulta inclui a coleção de itens do pedido (\texttt{.Include(p => p.Itens)}).
            \item A agregação soma o campo correto (\texttt{.Sum(item => item.PrecoVenda)}).
            \item Os filtros de status e data funcionam corretamente para cálculos de faturamento e mapeamentos.
        \end{itemize}
\end{itemize}
