...
A arquitetura adotada no projeto segue o padrão MVC (Model-View-Controller), amplamente utilizado em aplicações web. O sistema é segmentado em camadas que se comunicam entre si de forma estruturada: interface (frontend), lógica de negócios (backend) e persistência de dados. Para isso, foram utilizadas tecnologias como ASP.NET MVC, Entity Framework, SQL Server e JJMasterData, entre outras. A definição clara dessas camadas permite maior modularidade, facilidade de manutenção e suporte à escalabilidade horizontal por meio da nuvem, com hospedagem na Azure.
